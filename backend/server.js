...
app.get('/api/products', async (req, res) => {
  try {
    ...
    if (isFree !== undefined) query = query.where('isFree', '==', isFree === 'true');
    if (isPremium !== undefined) query = query.where('isPremium', '==', isPremium === 'true');
    if (isFeatured !== undefined) query = query.where('isFeatured', '==', isFeatured === 'true');
    if (isTrending !== undefined) query = query.where('isTrending', '==', isTrending === 'true');
...
app.post('/api/admin/products', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const productData = {
      ...
      price: parseFloat(price) || 0, whatsappLink: whatsappLink.trim(),
      imageUrl: imageUrl || null, isFree: isFree || false, isPremium: isPremium || false,
      isFeatured: isFeatured || false, isTrending: isTrending || false,
      isVisible: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    };
...
app.put('/api/users/profile', verifyToken, async (req, res) => {
  try {
    ...
    const updatedUser = await db.collection(COLLECTIONS.USERS).doc(uid).get();
    res.json(successResponse({ uid, ...updatedUser.data() }, 'Profile updated'));
  } catch (error) {
...
app.get('/api/users/links', verifyToken, async (req, res) => {
  try {
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      links.push({
        orderId: doc.id, productTitle: data.productTitle,
...
app.post('/api/payments/initiate', verifyToken, async (req, res) => {
  try {
    ...
    const orderData = {
      userId: req.user.uid, productId, productTitle: product.title,
      amount: product.price, phone, paymentStatus: 'pending',
    ...
    res.json(successResponse({
      orderId: orderRef.id, amount: product.price,
      status: 'pending', message: 'Payment initiated. Please wait.'
    }, 'Payment initiated'));
  } catch (error) {
...
app.get('/api/payments/status/:orderId', verifyToken, async (req, res) => {
  try {
    ...
    res.json(successResponse({
      orderId, status: order.paymentStatus,
...
app.listen(PORT, () => {
  ...
  console.log('📱 Developer: +255 724 525 910');
  console.log('========================================');
});
