...
// =========================================================
// MIDDLEWARE: VERIFY TOKEN (WITH ADMIN CHECK)
// =========================================================
...
const verifyToken = async (req, res, next) => {
  try {
    ...
    if (!userDoc.exists) {
      // If user doesn't exist, create one (for admin)
    ...
    // For admin, allow access even with invalid token (for testing)
...
app.post('/api/admin/products', verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    // Log incoming data
    console.log('📦 Creating product:', { title, category, price, whatsappLink });
    ...
    res.status(201).json(successResponse({ id: docRef.id, ...productData }, 'Product created successfully'));
  } catch (error) {
